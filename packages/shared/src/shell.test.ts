...
describe("extractPathFromShellOutput", () => {
  it("extracts the path between capture markers", () => {
    expect(
      extractPathFromShellOutput("__OSSCODE_PATH_START__\n/opt/homebrew/bin:/usr/bin\n__OSSCODE_PATH_END__\n"),
...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    expect(args?.[1]).toContain("__OSSCODE_PATH_START__");
    expect(args?.[1]).toContain("__OSSCODE_PATH_END__");
