...
import { ProviderModels } from "./provider/Services/ProviderModels";
 ...
 */
export class Server extends ServiceMap.Service<Server, ServerShape>()("osscode/wsServer/Server") {}
...
> {
  ...
  const routeRequest = Effect.fnUntraced(function* (request: WebSocketRequest) {
    switch (request.body._tag) {
      ...
      case WS_METHODS.providersGetModels: {
        const body = stripRequestTag(request.body);
        return yield* providerModels.getModels(body.provider);
      }
