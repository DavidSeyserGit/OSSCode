...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "osscode-terminal-"));
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.OSSCODE_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    await manager.open(
      openInput({
        env: {
          OSSCODE_PROJECT_ROOT: "/repo",
          OSSCODE_WORKTREE_PATH: "/repo/worktree-a",
          CUSTOM_FLAG: "1",
        },
      }),
    );
    ...
    expect(spawnInput.env.OSSCODE_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.OSSCODE_WORKTREE_PATH).toBe("/repo/worktree-a");
