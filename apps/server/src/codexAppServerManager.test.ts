...
describe("startSession", () => {
  it("enables Codex experimental api capabilities during initialize", () => {
    expect(buildCodexInitializeParams()).toEqual({
      clientInfo: {
        name: "osscode_desktop",
        title: "OSS Code Desktop",
        version: "0.1.0",
      },
      capabilities: {
  ...
  it("fails fast with an upgrade message when codex is below the minimum supported version", async () => {
    ...
    const versionCheck = vi
      ...
      .mockImplementation(() => {
        throw new Error(
          "Codex CLI v0.36.0 is too old for OSS Code. Upgrade to v0.37.0 or newer and restart OSS Code.",
        );
      });
...
describe("sendTurn", () => {
  ...
  it("passes Codex ask mode as a plan collaboration preset with ask instructions", async () => {
    const { manager, context, sendRequest } = createSendTurnHarness();

    await manager.sendTurn({
      threadId: asThreadId("thread_1"),
      input: "What changed in this file?",
      interactionMode: "ask",
    });
  ...
  it("passes Codex debug mode as a default collaboration preset with debug instructions", async () => {
    const { manager, context, sendRequest } = createSendTurnHarness();
    ...
    expect(sendRequest).toHaveBeenCalledWith(context, "turn/start", {
      ...
      collaborationMode: {
        ...
        settings: {
          model: "gpt-5.3-codex",
          reasoning_effort: "medium",
          developer_instructions: CODEX_DEBUG_MODE_DEVELOPER_INSTRUCTIONS,
        },
      },
    });
  });
