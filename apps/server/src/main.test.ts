...
import { CliConfig, osscodeCli, recordStartupHeartbeat, type CliConfigShape } from "./main";
...
const runCli = (
  ...
  env: Record<string, string> = { OSSCODE_NO_BROWSER: "true" },
) => {
  const uniqueStateDir = `/tmp/t3-cli-state-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  return Command.runWith(osscodeCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            OSSCODE_STATE_DIR: uniqueStateDir,
            ...env,
          },
        }),
      ),
...
it.layer(testLayer)("server CLI command", (it) => {
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        ...
        OSSCODE_PORT: "4999",
        OSSCODE_HOST: "100.88.10.4",
        OSSCODE_STATE_DIR: "/tmp/t3-env-state",
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        OSSCODE_NO_BROWSER: "true",
        OSSCODE_AUTH_TOKEN: "env-token",
      });
  ...
  it.effect("prefers --mode over OSSCODE_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over OSSCODE_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        OSSCODE_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/log websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        OSSCODE_MODE: "desktop",
        OSSCODE_LOG_WS_EVENTS: "false",
        OSSCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        OSSCODE_NO_BROWSER: "true",
      });
